  ...
  try {
    ...
    if (req.headers['content-type']?.includes('multipart/form-data')) {
      ...
      console.log('Final processed form data:', {
        ...
        product_attributes:
          JSON.stringify(req.body.product_attributes).substring(0, 100) +
          (req.body.product_attributes.length > 3 ? '...' : ''),
        product_variations:
          JSON.stringify(req.body.product_variations).substring(0, 100) +
          (req.body.product_variations.length > 3 ? '...' : ''),
