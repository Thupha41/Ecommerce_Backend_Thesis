...
// Simple path matcher function (same as in rbac.middlewares.ts)
      ...
      code: {
        ...
        custom: {
          options: async (value: string, { req }) => {
            ...
            const isCodeValid = await databaseService.users.findOne({ code: req.body.code })
            if (!isCodeValid) {
              return {
                message: USERS_MESSAGES.INVALID_CODE
              }
            }
            const now = new Date()
            if (now > isCodeValid.code_expired) {
              return {
