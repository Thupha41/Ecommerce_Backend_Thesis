...
class RoleService {
    ...
    // Cập nhật role
    async updateRole(user_id: string, role_id: string, { role_name, role_description, role_grant, role_status }: IUpsertRole) {
        //check user
        const user = await databaseService.users.findOne({
            _id: new ObjectId(user_id)
        })
    ...
    async roleListRBAC(user_id: string) {
        ...
        //2. get list role
        const roles = await databaseService.roles.aggregate([
            {
                $unwind: '$role_grant'
            },
            {
