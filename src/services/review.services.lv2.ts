...
class ReviewService {
      ...
      .aggregate([
        { $match: { product_id: productId, status: ReviewStatus.Approved } },
        {
          $group: {
            ...
            reviews_with_media: {
              $sum: {
                $cond: [{ $gt: [{ $size: { $ifNull: ['$media', []] } }, 0] }, 1, 0]
              }
            },
            total_media_count: { $sum: { $size: { $ifNull: ['$media', []] } } },
            reviews_by_rating: {
              $push: {
                $cond: [{ $and: [{ $gte: ['$rating', 1] }, { $lte: ['$rating', 5] }] }, '$rating', null]
              }
            }
          }
        },
  ...
  async updateStatsAfterReview(reviewId: string) {
    ...
    try {
      ...
      const shopProducts = await databaseService.products.find({ product_shop: shopId, isPublished: true }).toArray()
  ...
  async updateReview(userId: string, reviewId: string, updateData: Partial<IUpsertReview> & { status?: ReviewStatus }) {
    ...
    if (user.role_ids && user.role_ids.length > 0) {
      const userRoles = await databaseService.roles.find({ _id: { $in: user.role_ids } }).toArray()
  ...
  async deleteReview(userId: string, reviewId: string) {
    ...
    if (foundUser.role_ids && foundUser.role_ids.length > 0) {
      const userRoles = await databaseService.roles.find({ _id: { $in: foundUser.role_ids } }).toArray()
      isAdmin = userRoles.some((role) => role.role_name.toLowerCase() === 'admin')
    }
    if (!isAdmin) {
      throw new ErrorWithStatus({
