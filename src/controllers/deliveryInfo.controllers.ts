...
export const getDeliveryDetail = async (req: Request, res: Response, next: NextFunction) => {
  const user = req.decoded_authorization as TokenPayload
  const user_id = user.user_id
  const result = await deliveryInfoService.getDeliveryDetail(user_id.toString(), req.params.delivery_id)
  res.json({
    message: DELIVERY_INFO_MESSAGES.GET_DELIVERY_DETAIL_SUCCESS,
    result
  })
  return
}

export const getDeliveryDefault = async (req: Request, res: Response, next: NextFunction) => {
  const user = req.decoded_authorization as TokenPayload
  const user_id = user.user_id
  const result = await deliveryInfoService.getDeliveryDefault(user_id.toString())
  res.json({
    message: DELIVERY_INFO_MESSAGES.GET_DELIVERY_DEFAULT_SUCCESS,
    result
  })
  return
}
