...
async function checkAndCreateIndex(
    ...
    try {
        ...
        // Kiểm tra tên index
        const indexExistsByName = existingIndexes.some((idx: any) =>
        ...
        const indexExists = existingIndexes.some((idx: any) => {
            // Kiểm tra key
            const keysMatch = Object.keys(indexSpec.key).every(
                (key) => idx.key[key] === indexSpec.key[key]
            );
        ...
        throw new Error(
            `Failed to process index ${indexSpec.name} on ${indexSpec.collection}: ${errorMessage}`
        );
    }
}
